...
const VolunteersList = ({ projectId, volunteers, fetchProjectVolunteers }) => {
  ...
  const openAssignModal = (volunteerId) => {
  ...
  return (
    <div className="bg-white p-4 rounded-md shadow-md mt-4">
            ...
            {volunteers.map((volunteer) => (
              <tr key={volunteer._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                  <button
                    onClick={() => openAssignModal(volunteer._id)}
      ...
      {showAssignModal && selectedVolunteerId && (
        <AssignVolunteerModal
          isOpen={showAssignModal}
          onClose={closeAssignModal}
          volunteerId={selectedVolunteerId}
          onAssignSuccess={() => fetchProjectVolunteers(projectId)}
        />
      )}

      {showAssignModal && selectedVolunteerId === null && (
        <AssignVolunteerModal
          isOpen={showAssignModal}
          onClose={closeAssignModal}
          volunteerId={null}
