...
const OutreachDashboard = () => {
  ...
  // Role access control
  if (!loading && (!user || (userRole !== 'outreach' && userRole !== 'admin'))) { // Consider using a dedicated route or component for access control
  ...
  useEffect(() => {
    const fetchOutreachData = async () => {
      setDataLoading(true);
      try {
        // Simulated fetch — replace with real fetch from Firestore or backend
        setTimeout(() => {
          setDashboardData({
            leads: [], // or [{ id, name, status }]
            events: [], // or [{ id, title, date }]
          });
          setError(null);
