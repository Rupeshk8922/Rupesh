...
const allowedRoles = ['admin', 'csr', 'project-manager']; // Adjust as needed
...
const CreateEventPage = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.maxVolunteers < 0)
      errors.maxVolunteers = 'Max volunteers cannot be negative';
    if (!formData.assignedOfficer)
      errors.assignedOfficer = 'Assigned officer is required';
      ...
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        ...
        <div>
          ...
          <Input
            ...
            min={0}
          />
          {formErrors.maxVolunteers && (
            <p className="text-sm text-red-600">{formErrors.maxVolunteers}</p>
          )}
        </div>
        ...
        <div>
          ...
          <OfficerSelect
            ...
            onChange={(val) =>
              setFormData((prev) => ({ ...prev, assignedOfficer: val }))
            }
            disabled={submissionLoading}
          />
          {formErrors.assignedOfficer && (
            <p className="text-sm text-red-600">{formErrors.assignedOfficer}</p>
          )}
        </div>
